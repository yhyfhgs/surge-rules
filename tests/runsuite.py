#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""runsuite.py — 分流测试套件 L2：场景断言运行器。

加载 scenarios/*.json，先做严格 schema 校验（非法场景整套拒载、退出 2，绝不静默
跳过），再逐请求调用同目录 engine.py 判定并核对断言。

场景 = 一次自然用户行为触发的整组域名；断言的是「这套操作会不会被拆到不同出口」，
而不只是单个域名走哪。schema 要点：
* name 全局唯一；requests 非空，host 至少两级、ip 是单个地址（不收 CIDR）；
* assert 至少含一项有效断言；policy 与 policy_in 互斥；
* per_request 按 (host, ip) 精确对上某个请求，撞键与孤儿都是错误；
* same_policy:true 需要至少 2 个未被 per_request 覆盖的请求，否则是假绿。

用法：python3 runsuite.py [--conf PATH] [--rules DIR] [--dir DIR] [--filter TEXT] [--json]
退出码：0 全部通过；1 有断言失败；2 场景 schema / 引擎不可用。
"""

import argparse
import ipaddress
import json
import os
import sys
import unicodedata

HERE = os.path.dirname(os.path.abspath(__file__))
DEFAULT_SCEN_DIR = os.path.join(HERE, "scenarios")
sys.path.insert(0, HERE)

import engine as engine_mod  # noqa: E402

QUERY_KEYS = ("host", "ip")

# schema 键白名单；未知键在加载期报错。
SCENARIO_KEYS = ("assert", "desc", "name", "note", "requests")
REQUEST_KEYS = ("host", "ip", "note")
ASSERT_KEYS = ("no_dns_leak", "per_request", "policy", "policy_in", "same_policy")
PERREQ_KEYS = ("host", "ip", "note", "policy", "policy_in", "reason")


class SuiteError(Exception):
    pass


class Engine(object):
    """engine.py 的进程内桥接 + 按查询缓存。"""

    def __init__(self, conf=None, rules=None):
        self.conf = conf
        self.rules = rules
        self._cache = {}
        try:
            self._eng = engine_mod.build_engine(conf, rules)
        except SystemExit as e:
            raise SuiteError(str(e))

    def query(self, q):
        key = tuple(q.get(k) for k in QUERY_KEYS)
        if key not in self._cache:
            res = self._eng.match(host=q.get("host"), ip=q.get("ip"))
            self._cache[key] = {
                "policy": res.get("policy"),
                "matched_rule": res.get("matched_rule"),
                "source": res.get("source"),
                "dns_leak": bool(res.get("dns_leak")),
                "dns_leak_at": res.get("dns_leak_at"),
            }
        return self._cache[key]


def qkey(d):
    return tuple(d.get(k) for k in QUERY_KEYS)


def qlabel(q):
    bits = []
    if q.get("host"):
        bits.append(q["host"])
    if q.get("ip"):
        bits.append("ip=" + q["ip"])
    return " ".join(bits) or "<空查询>"


# ---------------------------------------------------------------------------
# 加载期 schema 校验
# ---------------------------------------------------------------------------

_HOST_BAD_CHARS = set(" \t\r\n\f\v/\\:?#@*%&=+,;'\"()[]{}<>|^`~!$")


def check_host(h):
    """合法返回 None，否则返回错误说明。IP 字面量作 host 合法（HTTPDNS 场景）。"""
    if not isinstance(h, str) or not h:
        return "host 必须是非空字符串"
    if len(h) > 253:
        return "host 长度 %d 超过 253" % len(h)
    bad = sorted(set(c for c in h if c in _HOST_BAD_CHARS or ord(c) < 33))
    if bad:
        return "host %r 含非法字符 %r（只写主机名，不带 scheme/端口/路径/通配符）" % (h, "".join(bad))
    labels = (h[:-1] if h.endswith(".") else h).split(".")
    if len(labels) < 2:
        return "host %r 至少要两级标签（形如 example.com）" % h
    for lb in labels:
        if not lb:
            return "host %r 含空标签" % h
        if len(lb) > 63:
            return "host %r 的标签 %r 超过 63 字符" % (h, lb)
        if lb.startswith("-") or lb.endswith("-"):
            return "host %r 的标签 %r 不能以连字符开头或结尾" % (h, lb)
    return None


def check_ip(v):
    if not isinstance(v, str) or not v:
        return "ip 必须是非空字符串"
    if "/" in v:
        return "ip %r 是 CIDR；此处只能写单个地址" % v
    try:
        ipaddress.ip_address(v)
    except ValueError:
        return "ip %r 不是合法的 IPv4/IPv6 地址" % v
    return None


def _unknown_keys(obj, allowed, layer, tag, errors):
    extra = sorted(k for k in obj if k not in allowed)
    if extra:
        errors.append("%s：%s 出现未知键 %s；允许的键为 {%s}"
                      % (tag, layer, ", ".join(repr(k) for k in extra),
                         ", ".join(allowed)))


def _check_target(d, tag, errors):
    """校验一个查询目标（request 或 per_request 条目）的 host/ip。"""
    n0 = len(errors)
    ok = False
    for key, fn in (("host", check_host), ("ip", check_ip)):
        if key in d:
            e = fn(d[key])
            if e:
                errors.append("%s：%s" % (tag, e))
            else:
                ok = True
    if not ok and len(errors) == n0:
        errors.append("%s：必须含合法的 host 或 ip（键：%s）"
                      % (tag, ", ".join(sorted(d)) or "<空对象>"))
    for k in ("note", "reason"):
        if k in d and (not isinstance(d[k], str) or not d[k]):
            errors.append("%s：%s 必须是非空字符串" % (tag, k))


def validate_scenarios(files):
    """严格校验全部场景；返回错误字符串列表（空 = 通过）。"""
    errors = []
    seen_names = {}

    for fname, arr in files:
        for idx, scn in enumerate(arr):
            where = "%s[%d]" % (fname, idx)
            if not isinstance(scn, dict):
                errors.append("%s：场景必须是 JSON 对象" % where)
                continue
            name = scn.get("name")
            tag = "%s «%s»" % (where, name if isinstance(name, str) and name
                               else "<未命名>")

            _unknown_keys(scn, SCENARIO_KEYS, "场景级", tag, errors)
            if not isinstance(name, str) or not name.strip():
                errors.append("%s：name 缺失或不是非空字符串" % tag)
            elif name in seen_names:
                errors.append("%s：name 与 %s 重复，必须全局唯一" % (tag, seen_names[name]))
            else:
                seen_names[name] = where
            for k in ("desc", "note"):
                if k in scn and not isinstance(scn[k], str):
                    errors.append("%s：%s 必须是字符串" % (tag, k))

            reqs = scn.get("requests")
            req_keys, reqs_usable = [], False
            if not isinstance(reqs, list):
                errors.append("%s：requests 缺失或不是数组" % tag)
            elif not reqs:
                errors.append("%s：requests 为空——空场景产不出断言却显示为通过，是假绿" % tag)
            else:
                reqs_usable = True
                for ri, r in enumerate(reqs):
                    rtag = "%s request[%d]" % (tag, ri)
                    if not isinstance(r, dict):
                        errors.append("%s：请求必须是 JSON 对象" % rtag)
                        continue
                    _unknown_keys(r, REQUEST_KEYS, "request", rtag, errors)
                    _check_target(r, rtag, errors)
                    req_keys.append(qkey(r))

            asrt = scn.get("assert")
            if not isinstance(asrt, dict):
                errors.append("%s：assert 缺失或不是 JSON 对象" % tag)
                continue
            _unknown_keys(asrt, ASSERT_KEYS, "assert", tag, errors)
            if "policy" in asrt and "policy_in" in asrt:
                errors.append("%s：assert 的 policy 与 policy_in 互斥" % tag)
            if "policy" in asrt and (not isinstance(asrt["policy"], str)
                                     or not asrt["policy"]):
                errors.append("%s：assert.policy 必须是非空字符串" % tag)
            if "policy_in" in asrt:
                pin = asrt["policy_in"]
                if (not isinstance(pin, list) or not pin
                        or not all(isinstance(x, str) and x for x in pin)):
                    errors.append("%s：assert.policy_in 必须是非空的字符串数组" % tag)
            for k in ("same_policy", "no_dns_leak"):
                if k in asrt and not isinstance(asrt[k], bool):
                    errors.append("%s：assert.%s 必须是布尔值" % (tag, k))

            per = asrt.get("per_request", [])
            if "per_request" in asrt and not isinstance(per, list):
                errors.append("%s：assert.per_request 必须是数组" % tag)
                per = []
            per_keys = {}
            for pi, p in enumerate(per):
                ptag = "%s per_request[%d]" % (tag, pi)
                if not isinstance(p, dict):
                    errors.append("%s：条目必须是 JSON 对象" % ptag)
                    continue
                _unknown_keys(p, PERREQ_KEYS, "per_request", ptag, errors)
                _check_target(p, ptag, errors)
                has_p, has_pin = "policy" in p, "policy_in" in p
                if has_p and has_pin:
                    errors.append("%s：policy 与 policy_in 互斥" % ptag)
                elif not has_p and not has_pin:
                    errors.append("%s：条目必须给出 policy 或 policy_in" % ptag)
                if has_p and (not isinstance(p["policy"], str) or not p["policy"]):
                    errors.append("%s：policy 必须是非空字符串" % ptag)
                if has_pin:
                    pin = p["policy_in"]
                    if (not isinstance(pin, list) or not pin
                            or not all(isinstance(x, str) and x for x in pin)):
                        errors.append("%s：policy_in 必须是非空的字符串数组" % ptag)
                k = qkey(p)
                if k in per_keys:
                    errors.append("%s：与 per_request[%d] 撞键（%s），前一条断言会静默丢失"
                                  % (ptag, per_keys[k], qlabel(p)))
                else:
                    per_keys[k] = pi
                if reqs_usable and k not in req_keys:
                    errors.append("%s：条目 %s 对不上任何请求（按 (host, ip) 精确匹配）"
                                  % (ptag, qlabel(p)))

            if not (("policy" in asrt) or ("policy_in" in asrt)
                    or asrt.get("same_policy") is True
                    or asrt.get("no_dns_leak") is True
                    or bool(per)):
                errors.append("%s：assert 里没有任何有效断言（policy / policy_in / "
                              "same_policy:true / no_dns_leak:true / 非空 per_request）" % tag)

            if asrt.get("same_policy") is True:
                uncovered = [k for k in req_keys if k not in per_keys]
                if len(uncovered) < 2:
                    errors.append("%s：same_policy:true 但未被 per_request 覆盖的请求只有 %d 个"
                                  "——单请求的「同组」断言必然通过，是假绿" % (tag, len(uncovered)))
    return errors


# ---------------------------------------------------------------------------
# 执行
# ---------------------------------------------------------------------------

def run_scenario(scn, eng):
    """返回 (assertions, error)；assertion = dict(scenario, kind, query, expect, actual, ok)。"""
    out = []
    asrt = scn.get("assert", {}) or {}
    per_map = {qkey(p): p for p in (asrt.get("per_request") or [])}

    results = []
    for q in scn.get("requests", []):
        try:
            r = eng.query(q)
        except Exception as e:                                  # noqa: BLE001
            return out, "engine 查询失败（%s）：%s" % (qlabel(q), e)
        results.append((q, r))

    def add(kind, q, expect, actual, ok):
        out.append({"scenario": scn.get("name"), "kind": kind,
                    "query": qlabel(q) if q else "<场景级>",
                    "expect": expect, "actual": actual, "ok": ok})

    uncovered = []
    for q, r in results:
        entry = per_map.get(qkey(q))
        detail = "%s [%s @ %s]" % (r["policy"], r["matched_rule"], r["source"])
        if entry is not None:
            if "policy_in" in entry:
                add("per_request.policy_in", q, entry["policy_in"], detail,
                    r["policy"] in entry["policy_in"])
            elif "policy" in entry:
                add("per_request.policy", q, entry["policy"], detail,
                    r["policy"] == entry["policy"])
        else:
            uncovered.append((q, r))
            if "policy" in asrt:
                add("policy", q, asrt["policy"], detail, r["policy"] == asrt["policy"])
            elif "policy_in" in asrt:
                add("policy_in", q, asrt["policy_in"], detail,
                    r["policy"] in asrt["policy_in"])

    if asrt.get("same_policy"):
        pols = []
        for _q, r in uncovered:
            if r["policy"] not in pols:
                pols.append(r["policy"])
        add("same_policy", None, "所有请求同一策略组",
            " / ".join(str(p) for p in pols) if pols else "<无请求>", len(pols) <= 1)

    if asrt.get("no_dns_leak"):
        for q, r in results:
            add("no_dns_leak", q, "无本地解析",
                ("泄漏于 " + str(r["dns_leak_at"])) if r["dns_leak"] else "无泄漏",
                not r["dns_leak"])

    return out, None


def load_scenarios(scen_dir):
    if not os.path.isdir(scen_dir):
        raise SuiteError("场景目录不存在：%s（用 --dir 指定）" % scen_dir)
    files = sorted(f for f in os.listdir(scen_dir) if f.endswith(".json"))
    if not files:
        raise SuiteError("场景目录里没有 .json 文件：%s" % scen_dir)
    data = []
    for f in files:
        p = os.path.join(scen_dir, f)
        try:
            with open(p, encoding="utf-8") as fh:
                arr = json.load(fh)
        except Exception as e:                                  # noqa: BLE001
            raise SuiteError("场景文件解析失败 %s：%s" % (p, e))
        if not isinstance(arr, list):
            raise SuiteError("场景文件顶层必须是数组：%s" % p)
        data.append((f, arr))
    return data


# ---------------------------------------------------------------------------
# 输出
# ---------------------------------------------------------------------------

def dwidth(s):
    return sum(2 if unicodedata.east_asian_width(c) in "WF" else 1 for c in str(s))


def pad(s, n):
    s = str(s)
    return s + " " * max(0, n - dwidth(s))


def summarize(rows):
    s = {"total": len(rows), "pass": 0, "fail": 0}
    for r in rows:
        s["pass" if r["ok"] else "fail"] += 1
    return s


def report(per_file, all_asserts, stats, args):
    print("=" * 78)
    print("Surge 分流场景回归（L2 runsuite）")
    print("=" * 78)
    print("配置     : %s" % (args.conf or "<引擎默认>"))
    print("场景目录 : %s" % args.dir)
    if args.filter:
        print("过滤     : %s" % args.filter)
    print("")

    cols = [("文件", 24), ("场景", 6), ("请求", 6), ("断言", 6), ("通过", 6), ("失败", 6)]
    print("".join(pad(c, w) for c, w in cols))
    print("-" * 78)
    for fname, nscn, nreq, rows in per_file:
        if nscn == 0:
            continue
        s = summarize(rows)
        print("".join(pad(v, w) for v, (_, w) in zip(
            [fname, nscn, nreq, s["total"], s["pass"], s["fail"]], cols)))
    print("-" * 78)
    print("".join(pad(v, w) for v, (_, w) in zip(
        ["合计", sum(x[1] for x in per_file), sum(x[2] for x in per_file),
         stats["total"], stats["pass"], stats["fail"]], cols)))
    print("")

    fails = [r for r in all_asserts if not r["ok"]]
    if fails:
        print("【失败明细】")
        for r in fails:
            print("  ✗ %s / %s" % (r["scenario"], r["kind"]))
            print("      查询: %s" % r["query"])
            print("      期望: %s" % r["expect"])
            print("      实际: %s" % r["actual"])
        print("")

    leak_all = [r for r in all_asserts if r["kind"] == "no_dns_leak"]
    leaks = [r for r in leak_all if not r["ok"]]
    print("DNS 泄漏断言: %d 条，失败 %d 条%s"
          % (len(leak_all), len(leaks), "（P1 实锤，需立即修）" if leaks else " ✓"))
    print("结果: %s（失败 %d / 通过 %d）"
          % ("FAIL" if stats["fail"] else "PASS", stats["fail"], stats["pass"]))


def main():
    ap = argparse.ArgumentParser(description="Surge 分流场景断言运行器（L2）")
    ap.add_argument("--dir", default=DEFAULT_SCEN_DIR, help="场景目录（默认 ./scenarios）")
    ap.add_argument("--conf", default=None, help="Surge.conf 路径（透传给引擎）")
    ap.add_argument("--rules", default=None, help=".list 目录（透传给引擎）")
    ap.add_argument("--filter", default=None, help="只跑名字或文件名含该子串的场景")
    ap.add_argument("--json", action="store_true", dest="as_json", help="机器可读输出")
    args = ap.parse_args()

    try:
        files = load_scenarios(args.dir)
    except SuiteError as e:
        sys.stderr.write("错误：%s\n" % e)
        return 2

    schema_errors = validate_scenarios(files)
    if schema_errors:
        sys.stderr.write("场景 schema 校验失败 %d 项，套件拒绝加载（未跑任何断言）：\n"
                         % len(schema_errors))
        for e in schema_errors:
            sys.stderr.write("  ✗ %s\n" % e)
        return 2

    try:
        eng = Engine(args.conf, args.rules)
    except SuiteError as e:
        sys.stderr.write("错误：%s\n" % e)
        return 2

    per_file, all_asserts = [], []
    for fname, arr in files:
        rows, nscn, nreq = [], 0, 0
        for scn in arr:
            name = scn.get("name", "")
            if args.filter and args.filter not in name and args.filter not in fname:
                continue
            a, err = run_scenario(scn, eng)
            if err:
                sys.stderr.write("错误：%s\n" % err)
                return 2
            rows.extend(a)
            nscn += 1
            nreq += len(scn.get("requests", []))
        per_file.append((fname, nscn, nreq, rows))
        all_asserts.extend(rows)

    stats = summarize(all_asserts)
    if args.as_json:
        print(json.dumps({
            "conf": args.conf, "rules": args.rules, "totals": stats,
            "files": [{"file": f, "scenarios": ns, "requests": nr,
                       "stats": summarize(rows)} for f, ns, nr, rows in per_file],
            "assertions": all_asserts,
        }, ensure_ascii=False, indent=2))
    else:
        report(per_file, all_asserts, stats, args)
    return 1 if stats["fail"] else 0


if __name__ == "__main__":
    sys.exit(main())
